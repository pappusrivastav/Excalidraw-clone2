...
canvas.width = 1500; // Set the width in pixels
canvas.height = 800; // Set the height in pixels
...
zoomActionButtons.forEach(button => {
    button.addEventListener('click', function () {
        ...
        if (action === 'in') {
            zoomLevel += 50; // Increase zoom level by 10%
        } else if (action === 'out') {
            zoomLevel -= 50; // Decrease zoom level by 10%
        }

        if (zoomLevel < 50) {
            zoomLevel = 50; // Set a minimum zoom level
        }
